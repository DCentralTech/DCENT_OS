#!/bin/sh
#
# DCENTos post-build script — am3-s19jpro-aml (S19j Pro Amlogic PIC1704 variant)
...
# base layer. Differs only in product identity (BM1362 + PIC1704 voltage
# controller, vs S21's BM1368 + NoPic TAS5782M).
#
...
#
# CRITICAL — :
#   board_target MUST be "am3-s19jpro-aml" here. Sysupgrade tarball prefix is
#   "sysupgrade-am3-s19jpro-aml/". Wrong name = silent failure on flash.
#
set -e
TARGET_DIR=$1
...
# served by server.py. Single source of truth is dashboard/dist/index.html
# (`cd DCENT_OS_Antminer/dashboard && npm run build`).
...
if [ -f "$DASHBOARD_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): installed dashboard SPA ($DASHBOARD_SIZE bytes) at /usr/share/dcentos-dashboard/index.html"
    if [ "$DASHBOARD_SIZE" -lt 100000 ]; then
        echo "DCENTos post-build (am3-s19jpro-aml): ERROR: dashboard appears truncated ($DASHBOARD_SIZE bytes < 100 KB floor)" >&2
        echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
        exit 1
    fi
else
    echo "DCENTos post-build (am3-s19jpro-aml): ERROR: dashboard not found at $DASHBOARD_SRC" >&2
    echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
    exit 1
fi
...
if [ -f "$DCENTOS_INIT" ]; then
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): installed dcentos-init as /sbin/init"
else
    echo "DCENTos post-build (am3-s19jpro-aml): WARNING: dcentos-init not found at $DCENTOS_INIT"
fi
...
if [ -f "$DCENTRALD_BIN" ]; then
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): installed dcentrald ($DCENTRALD_SIZE bytes)"
else
    echo "DCENTos post-build (am3-s19jpro-aml): ERROR: dcentrald not found at $DCENTRALD_BIN" >&2
    exit 1
fi

STAGED_BIN="${TARGET_DIR}/usr/local/bin/dcentrald"
if [ ! -f "$STAGED_BIN" ]; then
    echo "DCENTos post-build (am3-s19jpro-aml): ERROR: dcentrald missing from $STAGED_BIN" >&2
    exit 1
fi
...
dcent_require_dcentrald_version_match \
    "$TARGET_DIR" \
    "$STAGED_BIN" \
    "DCENTos post-build (am3-s19jpro-aml)" \
    "${BR2_EXTERNAL_DCENTOS_PATH}/../dcentrald/Cargo.toml"

if command -v md5sum > /dev/null 2>&1; then
    md5sum "$STAGED_BIN" | cut -d' ' -f1 > "${TARGET_DIR}/etc/dcentos/dcentrald.md5"
    echo "DCENTos post-build (am3-s19jpro-aml): md5 $(cat "${TARGET_DIR}/etc/dcentos/dcentrald.md5")"
fi

if [ ! -f "${TARGET_DIR}/etc/dcentrald.toml" ]; then
    echo "DCENTos post-build (am3-s19jpro-aml): ERROR: /etc/dcentrald.toml missing from rootfs." >&2
    echo "  Expected board/amlogic/am3-s19jpro-aml/rootfs-overlay/etc/dcentrald.toml" >&2
    exit 1
fi

# F-9 (Sweep-v3 PR-086): board_family must match the S99verify
# detect_platform() `am3-aml*` classifier glob. Bare "am3" hit the
# `*) PLATFORM=unknown` arm — and because board_family was set (not
# "unknown"/empty) the board_target fallback never ran — silently
# disabling the per-family V5/V6/V7/V11/V12/V13 cold-boot-proof
# overrides. Stamp the SKU-qualified `am3-aml-*` form (same value as
# the platform file below; documented BOARD_FAMILY vocabulary;
# classifier-correct; preserves SKU granularity).
echo "am3-aml-s19jpro"   > "${TARGET_DIR}/etc/dcentos/board_family"
echo "am3-s19jpro-aml"   > "${TARGET_DIR}/etc/dcentos/board_target"
echo "am3-aml-s19jpro"   > "${TARGET_DIR}/etc/dcentos/platform"
...
if [ -f "$AM3_GEOMETRY_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): installed am3_geometry.sh for revert helpers"
else
    echo "DCENTos post-build (am3-s19jpro-aml): WARNING: am3_geometry.sh not found at $AM3_GEOMETRY_SRC" >&2
fi

# Phase 4C deliverable. The script may not exist yet when Phase 4B
# scaffolding lands; that's fine — warn but do not fail. Once Phase 4C
# ships revert_to_stock_am3_aml_s19jpro.sh, this becomes a hard install.
REVERT_S19JPRO_SRC="${BR2_EXTERNAL_DCENTOS_PATH}/../scripts/revert_to_stock_am3_aml_s19jpro.sh"
if [ -f "$REVERT_S19JPRO_SRC" ]; then
    cp "$REVERT_S19JPRO_SRC" "${TARGET_DIR}/usr/sbin/revert_to_stock_am3_aml_s19jpro.sh"
    chmod +x "${TARGET_DIR}/usr/sbin/revert_to_stock_am3_aml_s19jpro.sh" 2>/dev/null || true
    echo "DCENTos post-build (am3-s19jpro-aml): installed revert_to_stock_am3_aml_s19jpro.sh from scripts/"
else
    echo "DCENTos post-build (am3-s19jpro-aml): WARNING: revert_to_stock_am3_aml_s19jpro.sh not found (Phase 4C pending) at $REVERT_S19JPRO_SRC" >&2
fi

# Stock-Bitmain manifest (parity with zynq board post-build). The daemon
# probes /etc/dcentos/ first.
...
if [ -f "$MANIFEST_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): installed stock-bitmain-manifest.json"
fi

if [ -n "${DCENT_RELEASE_PUBKEY_FILE:-}" ]; then
    if [ ! -f "${DCENT_RELEASE_PUBKEY_FILE}" ]; then
        echo "DCENTos post-build (am3-s19jpro-aml): ERROR: release public key not found at ${DCENT_RELEASE_PUBKEY_FILE}" >&2
        exit 1
    fi
    ...
    echo "DCENTos post-build (am3-s19jpro-aml): ERROR: DCENT_REQUIRE_RELEASE_KEY=1 but DCENT_RELEASE_PUBKEY_FILE is not set" >&2
    exit 1
else
    echo "DCENTos post-build (am3-s19jpro-aml): WARNING: no release public key embedded (lab-only image)"
fi

echo "DCENTos post-build (am3-s19jpro-aml): directories, permissions, and board identity set."
...
dcent_provision_release_image "$TARGET_DIR" "am3-s19jpro-aml"
