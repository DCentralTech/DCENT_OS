...
[ -r "$ARCHIVE_ADMISSION_SRC" ] || {
    echo "DCENTos post-build (am2-s17pro): ERROR: archive-admission helper not found at $ARCHIVE_ADMISSION_SRC" >&2
    exit 1
}
...
[ -r "$ZYNQ_GEOMETRY_SRC" ] || {
    echo "DCENTos post-build (am2-s17pro): ERROR: Zynq geometry helper not found at $ZYNQ_GEOMETRY_SRC" >&2
    exit 1
}
