#!/bin/sh
#
# DCENTos post-image script - am3-s21pro (S21 Amlogic NoPic variant)
#
# Produces dcentos-sysupgrade-am3-s21pro.tar with sysupgrade-am3-s21pro/.
#

set -e

BINARIES_DIR="${BINARIES_DIR:-${BASE_DIR}/images}"
BOARD_NAME="am3-s21pro"
BOARD_FAMILY="am3"
OUTPUT_TAR="${BINARIES_DIR}/dcentos-sysupgrade-am3-s21pro.tar"

echo "=== DCENTos Post-Image Builder (am3-s21pro) ==="
echo ""
...
if [ ! -f "$ROOTFS_CPIO" ]; then
    ...
    echo "  in the am3-s21pro defconfig." >&2
    exit 1
fi
...
infer_package_version() {
    ...
    for candidate in \
        ...
        "${PROJECT_ROOT}/br2_external_dcentos/board/amlogic/am3-s21pro/rootfs-overlay/etc/dcentos-version" \
...
cat > "$SUP_DIR/MANIFEST.json" << EOF
{
  ...
  "toolbox": {
    "install_command": "dcent install <ip> -f dcentos-sysupgrade-am3-s21pro.tar",
    "update_command": "dcent install <ip> -f dcentos-sysupgrade-am3-s21pro.tar",
...
DCENT_TOOLBOX_INSTALL_COMMAND="dcent install <ip> -f dcentos-sysupgrade-am3-s21pro.tar"
...
echo ""
echo "=== Build Complete (am3-s21pro) ==="
