...
# --- Generate the backup plan ---
{
    ...
    echo "- Target class: am2 Zynq XC7Z020 BraiinsOS dual-slot layout"
    ...
    echo
    echo "The AM2 backup CAN be executed from the live BraiinsOS / DCENT_OS"
    echo "instance because we never write to the active slot. \`nanddump\` is a"
    echo "pure read operation. However:"
    echo
    ...
    echo "   — \`nanddump\` reads the raw NAND through OOB skip-bad, but the"
    echo "   resulting image is NOT a clean filesystem snapshot. Use it only"
    echo "   as a slot-level restore image."
...
EOF
    ...
    if [ "$PLAN_READY" = "1" ]; then
        ...
        echo "3. Run \`am2_nand_backup_execute.sh\` on the miner"
    else
...
# --- Generate JSON manifest template ---
{
    ...
    printf '  "target": {\n'
    printf '    "class": "am2 Zynq XC7Z020",\n'
    printf '    "layout": "BraiinsOS dual-slot mtd7/mtd8"\n'
    printf '  },\n'
