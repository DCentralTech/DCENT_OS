...
BOARD_NAME="am3-t21"
BOARD_FAMILY="am3"
OUTPUT_TAR="${BINARIES_DIR}/dcentos-sysupgrade-am3-t21.tar"

echo "=== DCENTos Post-Image Builder (am3-t21) ==="
echo ""
...
if [ ! -f "$ROOTFS_CPIO" ]; then
    ...
    echo "  in the am3-t21 defconfig." >&2
    exit 1
fi
...
infer_package_version() {
    ...
    for candidate in \
        ...
        "${PROJECT_ROOT}/br2_external_dcentos/board/amlogic/am3-t21/rootfs-overlay/etc/dcentos-version" \
...
elif [ -f "${REPO_ROOT}/extractions/t21/kernel_uimage.bin" ]; then
    KERNEL="${REPO_ROOT}/extractions/t21/kernel_uimage.bin"
    KERNEL_SRC="extractions/t21"
...
if [ -z "$KERNEL" ]; then
    ...
    echo "    ${REPO_ROOT}/extractions/t21/kernel_uimage.bin" >&2
...
cat > "$SUP_DIR/MANIFEST.json" << EOF
{
  ...
  "toolbox": {
    "install_command": "dcent install <ip> -f dcentos-sysupgrade-am3-t21.tar",
    "update_command": "dcent install <ip> -f dcentos-sysupgrade-am3-t21.tar",
...
DCENT_TOOLBOX_INSTALL_COMMAND="dcent install <ip> -f dcentos-sysupgrade-am3-t21.tar"
