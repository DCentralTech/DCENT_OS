#!/usr/bin/env python3
"""Build a strict exact-unit AM3-BB NAND backup plan without target access.
...
from validate_am1_nand_backup import MAC_RE, SAFE_TARGET_RE, SHA256_RE, ValidationError
from validate_am3_bb_nand_backup import EXPECTED_LAYOUTS, LAYOUT_NAME, TARGET_CLASS
from validate_am3_bb_nand_backup_plan import MIN_FREE_MB, PLAN_TYPE, validate_plan
...
def validate_manifest(path: Path) -> list[tuple[int, str, int]]:
    try:
    ...
    expected = list(EXPECTED_LAYOUTS[LAYOUT_NAME])
    if rows != expected:
        raise ValidationError("manifest does not contain the exact ordered AM3-BB geometry")
    return rows
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--restore-artifact-proof", "--luxos-return-proof", dest="restore_proof", type=Path, required=True)
...
def build(args: argparse.Namespace, now: datetime) -> tuple[bytes, bytes]:
    ...
    plan = {
        ...
        "target": {"class": TARGET_CLASS, "layout": LAYOUT_NAME, "partition_count": len(partitions)},
        "nand_backup_execute_go": 0,
        "plan_ready": 1,
        "readback_verify": 1,
        "pre_flight": {
    ...
    markdown = f"""# AM3-BB NAND Backup Execution Plan

- Generated: `{generated}`
- Exact endpoint: `{args.expect_ip}`
- Exact MAC/HWID: `{expected_mac}` / `{args.expect_hwid}`
- Pinned SSH host key: `{args.expect_host_key_sha256}`
- Exact layout: `{LAYOUT_NAME}` ({len(partitions)} partitions, {total} bytes)
- JSON plan SHA256: `{receipt_hash}`
- `plan_ready=1`
- `nand_backup_execute_go=0`
- `nand_write_go=0`
- `persistent_install_go=0`
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        # The validated JSON is the executor's authority and commit marker.
        # Publish the non-executable human view first so an interrupted pair
        # can never leave a runnable plan without its companion review view.
        with CommitSignalGuard(
            "durable AM3-BB NAND backup plan publication", ValidationError
        ) as termination:
