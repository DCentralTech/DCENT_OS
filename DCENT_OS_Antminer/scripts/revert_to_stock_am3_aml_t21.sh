#!/bin/sh
#
# revert_to_stock_am3_aml_t21.sh — Revert a T21 (Amlogic A113D, BM1368,
# BHB42611/BHB56902 hashboards) from DCENTos back to stock Bitmain firmware.
#
# ============================================================================
...
# ============================================================================
#
# T21 shares the Amlogic A113D platform + BM1368 silicon with S21. The
# flash mechanism, uImage magic, NAND rootfs window, U-Boot env layout are
# all identical (per scripts/lib/am3_geometry.sh). The product-family
# validation string is `t21-amlogic` (vs S21's `s21-amlogic`); psu profile
# is different (APW12+ register catalog applies to both, but T21 ships in a
# different power envelope) — but PSU concerns are not part of the
# revert-to-stock flash flow.
#
# `verified_revertable: false` in PROFILE_TABLE.amlogic-a113d-bm1368-t21.
# This script is CODE-COMPLETE but NOT live-tested.
#
# ============================================================================
# EXPECTED FLEET STATE
# ============================================================================
#
# Before:    Unit running DCENT_OS on am3-aml-t21 hardware (post install from
#            stock Bitmain T21).
# After:     Unit boots stock Bitmain T21 firmware on next reboot.
# Recovery
# if revert
# fails:     The previous DCENTos rootfs window may still boot — restore from
#            /data/restore-backup-<ts>/ before rebooting if the operator
#            wants to abort the revert.
#
# Usage:
#   DCENT_REVERT_AUTHORIZED=1 ./revert_to_stock_am3_aml_t21.sh \
#       --operator-acknowledged-data-loss \
#       --firmware Antminer-T21-AML-release-XXXXX.tar.gz \
#       --target <t21-ip> \
#       [--sha256 <expected>]
#
#   ./revert_to_stock_am3_aml_t21.sh --dry-run \
#       --firmware ./stock-t21-aml.tar.gz --target <t21-ip>
...
revert_init "am3_aml_t21" "DCENT_REVERT_AUTHORIZED"

PRODUCT_FAMILY="t21-amlogic"
...
Usage: $0 [options]
  ...
  --firmware <path>                  Stock Bitmain T21 AML tarball.
...
if [ "$REVERT_DRY_RUN" != "1" ]; then
    ...
    if [ -r /etc/dcentos/board_target ]; then
        ...
        case "$BOARD_TARGET" in
            am3-aml-t21|amlogic-a113d-bm1368-t21|t21-amlogic)
