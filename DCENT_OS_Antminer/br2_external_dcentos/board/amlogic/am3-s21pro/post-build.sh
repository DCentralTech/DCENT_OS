#!/bin/sh
#
# DCENTos post-build script - am3-s21pro (S21 Amlogic NoPic variant)
#
...
if [ -f "$DASHBOARD_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed dashboard SPA ($DASHBOARD_SIZE bytes) at /usr/share/dcentos-dashboard/index.html"
    if [ "$DASHBOARD_SIZE" -lt 100000 ]; then
        echo "DCENTos post-build (am3-s21pro): ERROR: dashboard appears truncated ($DASHBOARD_SIZE bytes < 100 KB floor)" >&2
        echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
        exit 1
    fi
else
    echo "DCENTos post-build (am3-s21pro): ERROR: dashboard not found at $DASHBOARD_SRC" >&2
    echo "  Run: cd DCENT_OS_Antminer/dashboard && npm run build" >&2
    exit 1
fi
...
if [ -f "$DCENTOS_INIT" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed dcentos-init as /sbin/init"
else
    echo "DCENTos post-build (am3-s21pro): WARNING: dcentos-init not found at $DCENTOS_INIT"
fi
...
if [ -f "$DCENTRALD_BIN" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed dcentrald ($DCENTRALD_SIZE bytes)"
else
    echo "DCENTos post-build (am3-s21pro): ERROR: dcentrald not found at $DCENTRALD_BIN" >&2
    exit 1
fi

STAGED_BIN="${TARGET_DIR}/usr/local/bin/dcentrald"
if [ ! -f "$STAGED_BIN" ]; then
    echo "DCENTos post-build (am3-s21pro): ERROR: dcentrald missing from $STAGED_BIN" >&2
    exit 1
fi
...
dcent_require_dcentrald_version_match \
    "$TARGET_DIR" \
    "$STAGED_BIN" \
    "DCENTos post-build (am3-s21pro)" \
    "${BR2_EXTERNAL_DCENTOS_PATH}/../dcentrald/Cargo.toml"

if command -v md5sum > /dev/null 2>&1; then
    md5sum "$STAGED_BIN" | cut -d' ' -f1 > "${TARGET_DIR}/etc/dcentos/dcentrald.md5"
    echo "DCENTos post-build (am3-s21pro): md5 $(cat "${TARGET_DIR}/etc/dcentos/dcentrald.md5")"
fi

if [ ! -f "${TARGET_DIR}/etc/dcentrald.toml" ]; then
    echo "DCENTos post-build (am3-s21pro): ERROR: /etc/dcentrald.toml missing from rootfs." >&2
    echo "  Expected board/amlogic/am3-s21pro/rootfs-overlay/etc/dcentrald.toml" >&2
    exit 1
fi
...
if [ -z "$BAKED_VOLTAGE" ] || [ "$BAKED_VOLTAGE" -gt 14500 ]; then
    echo "DCENTos post-build (am3-s21pro): ERROR: baked voltage_mv '${BAKED_VOLTAGE}' exceeds the 14500 mV chip-rail ceiling" >&2
    exit 1
fi
if [ -z "$BAKED_FAN" ] || [ "$BAKED_FAN" -gt 30 ]; then
    echo "DCENTos post-build (am3-s21pro): ERROR: baked fan_max_pwm '${BAKED_FAN}' exceeds the 30 PWM home cap" >&2
    exit 1
fi
echo "DCENTos post-build (am3-s21pro): safety re-verify OK (voltage_mv=${BAKED_VOLTAGE}<=14500, fan_max_pwm=${BAKED_FAN}<=30)"
...
# am3-t21; am3-s21pro + am3-s19kpro had the identical latent bug.
echo "am3-aml-s21pro"  > "${TARGET_DIR}/etc/dcentos/board_family"
echo "am3-s21pro"      > "${TARGET_DIR}/etc/dcentos/board_target"
echo "am3-aml-s21pro"  > "${TARGET_DIR}/etc/dcentos/platform"
...
if [ -f "$AM3_GEOMETRY_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed am3_geometry.sh for revert helpers"
else
    echo "DCENTos post-build (am3-s21pro): WARNING: am3_geometry.sh not found at $AM3_GEOMETRY_SRC" >&2
fi
...
if [ -f "$REVERT_S21_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed revert_to_stock_am3_aml_s21.sh from scripts/"
else
    echo "DCENTos post-build (am3-s21pro): WARNING: revert_to_stock_am3_aml_s21.sh not found at $REVERT_S21_SRC" >&2
fi
...
if [ -f "$MANIFEST_SRC" ]; then
    ...
    echo "DCENTos post-build (am3-s21pro): installed stock-bitmain-manifest.json"
fi

if [ -n "${DCENT_RELEASE_PUBKEY_FILE:-}" ]; then
    if [ ! -f "${DCENT_RELEASE_PUBKEY_FILE}" ]; then
        echo "DCENTos post-build (am3-s21pro): ERROR: release public key not found at ${DCENT_RELEASE_PUBKEY_FILE}" >&2
        exit 1
    fi
    ...
    echo "DCENTos post-build (am3-s21pro): ERROR: DCENT_REQUIRE_RELEASE_KEY=1 but DCENT_RELEASE_PUBKEY_FILE is not set" >&2
    exit 1
else
    echo "DCENTos post-build (am3-s21pro): WARNING: no release public key embedded (lab-only image)"
fi

echo "DCENTos post-build (am3-s21pro): directories, permissions, and board identity set."
...
dcent_provision_release_image "$TARGET_DIR" "am3-s21pro"
